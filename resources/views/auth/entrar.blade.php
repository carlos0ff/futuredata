            ...
            <form method="POST" action="{{ route('auth.entrar.post') }}" class="space-y-5">
                ...
                <div class="space-y-1.5" x-data="{ show: false }">
                    ...
                    <div class="relative">
                        ...
                        <input name="password" :type="show ? 'text' : 'password'" placeholder="••••••••" autocomplete="current-password" class="w-full pl-11 pr-11 py-3 text-sm rounded-xl border transition-all focus:outline-none {{ $errors->has('password') ? 'border-red-300 bg-red-50 text-red-700 placeholder-red-300 focus:border-red-400 focus:ring-2 focus:ring-red-100' : 'border-slate-200 bg-slate-50 text-slate-800 placeholder-slate-400 hover:border-slate-300 focus:border-slate-400 focus:bg-white focus:ring-2 focus:ring-slate-100' }}" />
