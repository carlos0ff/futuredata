...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
    ...
    <div class="flex flex-wrap items-center gap-2">
        {{-- Portal do cliente --}}
        @if($ordem->token)
        <a href="{{ route('portal.show', $ordem->token) }}" target="_blank"
            ...
            <div class="p-4 space-y-3">
                ...
                <div class="rounded-xl border border-blue-200 bg-white px-3.5 py-2.5">
                    ...
                    <div class="flex items-center gap-2">
                        <code class="flex-1 truncate text-[11.5px] text-blue-700 font-mono">{{ route('portal.show', $ordem->token) }}</code>
                        <button
                            @click="navigator.clipboard.writeText('{{ route('portal.show', $ordem->token) }}'); copied = true; setTimeout(() => copied = false, 2000)"
                ...
                @php $wa = 'https://wa.me/55'.preg_replace('/\D/','',$ordem->cliente->telefone).'?text='.urlencode('Olá! Acesse o portal para acompanhar sua OS: '.route('portal.show', $ordem->token)); @endphp
